...
CREATE(admin3:Administrateur{login:'a000003'})
//Appartenir
...
//Article
CREATE (Promo_enceintes :Article {intitule: 'Promo_enceintes',
    contenu:'Destockage massif de nos enceintes pour le nouvel an !',
    auteur: 'Bose',
    date_publication: '2017-12-24'}
)
...
CREATE (admin2)-[:met_en]->(Maintenance)
CREATE (admin2)-[:met_en]->(Candidature_acteur)
CREATE (admin3)-[:met_en]->(Rabais_eviers)
//promouvoit des produit vedette
CREATE (Promo_enceintes)-[:promouvoit]->(enceinte3)
CREATE (Canicule)-[:promouvoit]->(frigo02)
CREATE (Rabais_eviers)-[:promouvoit]->(evier02)
